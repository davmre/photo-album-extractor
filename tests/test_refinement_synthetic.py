...
DEBUG_IMAGES_BASE_DIR = None # "/Users/dave/photos_tests/"  # Edit to save debug images.
...
def axis_aligned_square():
    ...
    cv2.fillPoly(image, [true_corners - PIXEL_BOUNDS], (255, 255, 255), lineType=cv2.LINE_AA)
...
def adjacent_boxes():
    ...
    cv2.fillPoly(image, [true_corners1  - PIXEL_BOUNDS], (255, 255, 255), lineType=cv2.LINE_AA)
    cv2.fillPoly(image, [true_corners2  - PIXEL_BOUNDS], (255, 255, 255), lineType=cv2.LINE_AA)
...
def colored_edge():
    img_height = 400
    img_width = 400
    image = np.zeros((img_height,img_width,3), np.uint8) + np.array((255, 0, 0), dtype=np.uint8)
...
def hires_overlapping_boxes():
    ...
    image = np.zeros((img_height,img_width,3), np.uint8) + np.asarray([255, 255, 255], dtype=np.uint8)
    ...
    true_corners1 = np.asarray(true_corners1, dtype=int)
    true_corners2 = np.asarray(true_corners2, dtype=int)    
    cv2.fillPoly(image, [true_corners1 - PIXEL_BOUNDS], (100, 100, 100), lineType=cv2.LINE_AA)
    cv2.fillPoly(image, [true_corners2 - PIXEL_BOUNDS], (100, 100, 100), lineType=cv2.LINE_AA)
...
def rotate_around_center(points, angle):
    ...
    rotated_coords = np.dot(relative_coords, 
...
class TestSyntheticImageRefinement:
    ...
    def check_refinement_recovers_gold_corners(
        self, image, input_box, gold_box, reltol=0.05, allowed_deviation=3,
        resolution_scale_factor=1.,
        debug_name=""):
        refined_box = refine_bounds.refine_bounding_box(
            image, input_box, reltol=reltol, enforce_parallel_sides=True,
            resolution=200,
            debug_dir=(os.path.join(DEBUG_IMAGES_BASE_DIR, debug_name)
                       if DEBUG_IMAGES_BASE_DIR else None))
    ...
    @pytest.mark.parametrize(
        "test_generator_fn", 
        [axis_aligned_square, axis_aligned_rect, skewed_rect,
         diamond, adjacent_boxes, overlapping_boxes, one_corner_offscreen,
         colored_edge, hires_overlapping_boxes])
    ...
    def test_gold_corners_are_fixed_point(
        self, test_generator_fn, reltol=0.05, resolution_scale_factor=1.):
        image, gold_boxes, name = test_generator_fn()
        for box_idx, gold_box in enumerate(gold_boxes):
            refined_box = refine_bounds.refine_bounding_box(
                image, gold_box, reltol=reltol, enforce_parallel_sides=True,
                resolution=200,
                debug_dir=(os.path.join(DEBUG_IMAGES_BASE_DIR,
                                        f"{name}_box{box_idx}_gold_init")
                        if DEBUG_IMAGES_BASE_DIR else None))
            deviation = max_deviation(gold_box, refined_box)
...
class TestSyntheticImageRefinementStrips:
    ...
    @pytest.mark.parametrize(
        "test_generator_fn", 
        [axis_aligned_square, axis_aligned_rect, skewed_rect, almost_rect,
         diamond, adjacent_boxes, overlapping_boxes, one_corner_offscreen,
         two_corners_offscreen, colored_edge, hires_overlapping_boxes])
    def test_refinement_of_perturbed_corners_recovers_gold_corners(
        self, test_generator_fn, reltol=0.03, num_perturbations=4):
