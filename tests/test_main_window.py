...
class TestMainWindow:
    ...
    def test_load_image_from_path(
        self, app: PhotoExtractorApp, test_images_dir: str, qtbot
    ):
    ...
    def test_load_directory(self, app: PhotoExtractorApp, test_images_dir: str, qtbot):
        """Test loading a directory of images."""
        app.load_directory(Path(test_images_dir))
        ...
        first_image = Path(test_images_dir) / expected_files[0]
    ...
    def test_set_current_directory(
        ...
        app.set_current_directory(Path(test_images_dir))

        qtbot.wait(100)

        assert app.current_directory == Path(test_images_dir)
        assert app.bounding_box_storage is not None
        assert app.bounding_box_storage.directory == test_images_dir

        # Directory sidebar should be updated
        image_list = app.directory_list
        assert image_list.current_directory == Path(test_images_dir)
