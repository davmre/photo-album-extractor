...
GOLD_DATA = load_json(os.path.join(
        REFINEMENT_TEST_DATA_DIR, 'gold_boxes.json'))
INIT_DATA = load_json(os.path.join(
    REFINEMENT_TEST_DATA_DIR, 'init_boxes.json'))
...
def get_matched_corner_deviations(rect, gold_rects):
    deviations = np.array(
        [get_corner_deviations(rect, gold) for gold in gold_rects])
...
class ImageWithBoxes:
    ...
    def dump_debug_images_for_failures(
        self, debug_dir: str, refine_strategy = None,
        allowed_average_deviation = 2, allowed_max_deviation = 3):
        ...
        for strategy in strategies:
            ...
            for i in range(len(self.corner_deviations[strategy])):
                ...
                if (
                    np.max(box_deviations) > allowed_max_deviation or 
                    np.mean(box_deviations) > allowed_average_deviation
                ):
                    # Rerun refinement to dump debugging info
                    refine_fn(self.image, self.init_boxes[i], 
...
def main():
    strategies = ["Strips (native res)", 
                  "Original (multiscale)"]
    
    test_image_filenames = [
        name for name in os.listdir(REFINEMENT_TEST_DATA_DIR)
    ...
    for filename in test_image_filenames:
        ...
        for strategy in strategies:
            corner_deviations[strategy].extend(
                test_object.corner_deviations[strategy])
    ...
    for strategy in strategies:
        ...
        print(f"{strategy}: median {median : .2f} avg {avg_case : .2f} worst {worst_case : .2f}")
