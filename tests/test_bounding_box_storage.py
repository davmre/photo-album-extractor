        ...
        test_boxes = [
            {
                ...
                'id': 'test-box-1',
                'corners': [[10, 10], [100, 10], [100, 100], [10, 100]],
                'attributes': {'date_time': '2024-01-01', 'comments': 'Original comment'}
            }
        ]
        temp_storage.data['test_image.jpg'] = test_boxes
        
        # Update attributes
        new_attributes = {'date_time': '2024-01-15', 'comments': 'Updated comment', 'location': 'New field'}
        result = temp_storage.update_box_attributes('test_image.jpg', 'test-box-1', new_attributes)
        
        assert result is True
        
        # Verify update
        updated_attributes = temp_storage.get_box_attributes('test_image.jpg', 'test-box-1')
    ...
    def test_all_boxes_have_attributes(self, storage):
        ...
        for image in test_images:
            ...
            for box in boxes:
                ...
                assert 'date_time' in attributes, f"Box in {image} missing date attribute"
                assert 'comments' in attributes, f"Box in {image} missing comment attribute"
    ...
    def test_box_coordinates_within_image_bounds(self, test_directory):
        ...
        for image_name, (width, height) in image_dimensions.items():
            ...
            for box in boxes:
                ...
                for corner in corners:
                    x, y = corner
                    assert 0 <= x <= width, f"X coordinate {x} out of bounds for {image_name}"
                    assert 0 <= y <= height, f"Y coordinate {y} out of bounds for {image_name}"