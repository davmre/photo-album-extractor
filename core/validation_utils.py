...
def validate_bounding_box(box) -> list[ValidationIssue]:
    ...
    if app_settings.warn_nonstandard_aspect:
        short, long = sorted(geometry.dimension_bounds(box.corners))
        aspect_ratio_reciprocal = short / long if long > 0 else np.nan
        ...
        if (
            ...
            issues.append(
                ValidationIssue(
                    ...
                    severity=Severity.WARNING,
                    message=f"Aspect ratio {long / short if short > 0 else np.nan: .2f} is not a standard photo size; is the bounding box correct?",
                )
            )
